...
RSpec.describe Mutations::SubmissionTargetOpen, type: :request, graphql: :mutation do
  mutation_query! <<~GRAPHQL
  mutation SubmissionTargetOpen($input: SubmissionTargetOpenInput!) {
  ...
  let(:valid_mutation_shape) do
    gql.mutation(:submission_target_open) do |m|
  ...
  let(:empty_mutation_shape) do
    gql.empty_mutation :submission_target_open
  end

  shared_examples_for "a successful mutation" do
    let(:expected_shape) { valid_mutation_shape }

    it "opens the submission target" do
      expect_request! do |req|
        req.effect! change { submission_target.current_state(force_reload: true) }.from("closed").to("open")
    ...
    context "when it is already open" do
      ...
      let(:expected_shape) do
        gql.mutation(:submission_target_open, no_errors: false) do |m|
          m[:submission_target] = be_blank

          m.global_errors do |ge|
            ge.error :unavailable_transition, message_args: { value: "open" }
          end
        end
      end

...
RSpec.describe Mutations::FrontendCacheRevalidateInstance, type: :request, graphql: :mutation do
  mutation_query! <<~GRAPHQL
  mutation FrontendCacheRevalidateInstance($input: FrontendCacheRevalidateInstanceInput!) {
  ...
  let_it_be(:revalidator_klass) { Frontend::Cache::InstanceRevalidator }

  let_it_be(:endpoint) { revalidator_klass.endpoint }
  let_it_be(:kind) { revalidator_klass.kind }

  let(:valid_mutation_shape) do
    gql.mutation(:frontend_cache_revalidate_instance) do |m|
      m[:revalidated] = true
    end
  end

  let(:empty_mutation_shape) do
    gql.empty_mutation :frontend_cache_revalidate_instance
  end
  ...
  shared_examples_for "an authorized mutation" do
    context "when the request is successful" do
      ...
      it "revalidates the instance" do

...
RSpec.describe "Query.submissionComment", type: :request do
  let(:query) do
    <<~GRAPHQL
    query getSubmissionComment($slug: Slug!) {
      submissionComment(slug: $slug) {
        id
        slug
  ...
  let(:found_shape) do
    gql.query do |q|
      q.prop :submission_comment do |m|
  ...
  let(:blank_shape) do
    gql.query do |q|
      q[:submission_comment] = be_blank
    end
  end

  let_it_be(:existing_model, refind: true) { FactoryBot.create :submission_comment }
  ...
  shared_examples "a found record" do
    it "finds the SubmissionComment" do
  ...
  shared_examples "a not found record" do
    it "does not find the SubmissionComment" do
      expect_request! do |req|

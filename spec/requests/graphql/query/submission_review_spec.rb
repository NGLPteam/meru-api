...
RSpec.describe "Query.submissionReview", type: :request do
  let(:query) do
    <<~GRAPHQL
    query getSubmissionReview($slug: Slug!) {
      submissionReview(slug: $slug) {
        id
        slug
  ...
  let(:blank_shape) do
    gql.query do |q|
      q[:submission_review] = be_blank
    end
  end

  let_it_be(:existing_model, refind: true) { FactoryBot.create :submission_review }
  ...
  shared_examples "a found record" do
    it "finds the SubmissionReview" do
  ...
  shared_examples "a not found record" do
    it "does not find the SubmissionReview" do
      expect_request! do |req|

...
RSpec.describe "Query.depositorRequests", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getDepositorRequestCollection($order: DepositorRequestOrder) {
        depositorRequests(order: $order) {
          edges {
            node {
              id
              slug
    ...
    let(:expected_shape) do
      gql.query do |q|
        q.prop :depositor_requests do |c|
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:depositor_request, **attrs)
      end
    end

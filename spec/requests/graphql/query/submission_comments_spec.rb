...
RSpec.describe "Query.submissionComments", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getSubmissionCommentCollection($order: SubmissionCommentOrder) {
        submissionComments(order: $order) {
          edges {
            node {
              id
              slug
    ...
    let(:expected_shape) do
      gql.query do |q|
        q.prop :submission_comments do |c|
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:submission_comment, **attrs)
      end
    end

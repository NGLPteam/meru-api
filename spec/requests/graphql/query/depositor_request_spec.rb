...
RSpec.describe "Query.depositorRequest", type: :request do
  let(:query) do
    <<~GRAPHQL
    query getDepositorRequest($slug: Slug!) {
      depositorRequest(slug: $slug) {
        id
        slug
  ...
  let(:blank_shape) do
    gql.query do |q|
      q[:depositor_request] = be_blank
    end
  end

  let_it_be(:existing_model, refind: true) { FactoryBot.create :depositor_request }
  ...
  shared_examples "a found record" do
    it "finds the DepositorRequest" do
  ...
  shared_examples "a not found record" do
    it "does not find the DepositorRequest" do
      expect_request! do |req|

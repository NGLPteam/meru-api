...
RSpec.describe "Query.submissionReviews", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getSubmissionReviewCollection($order: SubmissionReviewOrder) {
        submissionReviews(order: $order) {
          edges {
            node {
              id
              slug
    ...
    let(:expected_shape) do
      gql.query do |q|
        q.prop :submission_reviews do |c|
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:submission_review, **attrs)
      end
    end
